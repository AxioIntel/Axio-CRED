...
export interface ChangeSignal {id:string;entity:MonitoredEntity;type:"low_review"|"new_review"|"missing_reviews"|"count_decrease"|"burst"|"profile"|"rating";title:string;detail:string;at:string;from:string;datasetId:string;listingId:string;reviewId?:string}
...
export function detectChanges(entities:MonitoredEntity[],datasets:Dataset[]):ChangeSignal[]{
  ...
  for(const entity of entities){if(!entity.latest)continue;const history=snapshotHistory(entity.latest.listing,datasets);if(history.length<2)continue;const after=history.at(-1)!,before=history.at(-2)!;const elapsed=Date.parse(after.at)-Date.parse(before.at);if(elapsed<=0)continue;
    ...
    for(const r of after.listing.reviews)if(before.provider===after.provider&&!seen.has(r.id))signals.push({...base,id:`${prefix}:review:${r.id}`,reviewId:r.id,type:r.rating!==null&&r.rating<=2?"low_review":"new_review",title:`${r.rating===null?"Unrated":`${r.rating}-star`} review newly observed`,detail:`${r.author}: ${r.text??"No review text collected."} This review was not in earlier loaded samples; its posting time is not established by this comparison.`});
    if(before.provider===after.provider){
      const currentIds=new Set(after.listing.reviews.map(r=>r.id));
      const missing=before.listing.reviews.filter(r=>!currentIds.has(r.id));
      if(missing.length)signals.push({...base,id:prefix+":missing",datasetId:before.datasetId,listingId:before.listing.id,type:"missing_reviews",title:missing.length+" previously collected reviews not seen in this check",detail:"These records appeared in the previous snapshot but are absent from this collection. Partial coverage, sorting or access changes can cause this; removal is not confirmed. Open the previous evidence to investigate."});
    }
    const countBefore=before.listing.reviewCount,countAfter=after.listing.reviewCount;
    if(countBefore!==null&&countAfter!==null&&countAfter<countBefore)signals.push({...base,id:prefix+":count-decrease",type:"count_decrease",title:"Reported review count decreased: "+countBefore+" → "+countAfter,detail:"The platform reports fewer reviews. This does not identify which reviews were removed or why; compare the saved evidence and verify on the source."});
