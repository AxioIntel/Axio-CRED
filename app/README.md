...
Open `/collections` for batch discovery, center/grid searches, language, website-email and extended-review options. Inspect saved datasets in the review library, then select businesses to protect or monitor. The native scraper is primary; broad collection never invokes Outscraper. See [native feature coverage and architecture](NATIVE-COLLECTION-ARCHITECTURE.md) for configuration, limits and control-plane boundaries.
